    ...
    try {
      ...
      for (const p of paths) {
        try {
          ...
          if (res.ok || res.status === 401) return true; // 401 = gateway is up, just needs auth
        } catch {
          // try next
        }
      }
...
proxy.on("error", (err, _req, res) => {
  ...
  if (res && !res.headersSent && typeof res.writeHead === "function") {
    ...
    res.end("Gateway unavailable. It may be starting up -- refresh in a few seconds.");
  }
});
...
for (const sig of ["SIGTERM", "SIGINT"]) {
  process.on(sig, () => {
    console.log(`[wrapper] received ${sig}, shutting down`);
    server.close();
    if (gatewayProc) {
      gatewayProc.kill("SIGTERM");
      // Give the gateway 5s to exit gracefully, then force kill.
